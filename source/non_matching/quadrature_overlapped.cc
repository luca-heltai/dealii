...
namespace NonMatching
{
  ...
  compute_quadrature_on_intersection<1, 1, 1>(
    const typename dealii::Triangulation<1, 1>::cell_iterator &cell0,
    const typename dealii::Triangulation<1, 1>::cell_iterator &cell1,
    const unsigned int                                         degree,
  ...
  compute_quadrature_on_intersection<2, 1, 2>(
    const typename dealii::Triangulation<2, 2>::cell_iterator &cell0,
    const typename dealii::Triangulation<1, 2>::cell_iterator &cell1,
    const unsigned int                                         degree,
    ...
    const Mapping<1, 2>                                       &mapping1)
  {
    ...
    return QGaussSimplex<1>(degree).mapped_quadrature<1, 2>(vec_of_simplices);
  }
  ...
  compute_quadrature_on_intersection<2, 2, 2>(
    const typename dealii::Triangulation<2, 2>::cell_iterator &cell0,
    const typename dealii::Triangulation<2, 2>::cell_iterator &cell1,
    const unsigned int                                         degree,
    const Mapping<2, 2>                                       &mapping0,
    const Mapping<2, 2>                                       &mapping1)
  {
    const std::vector<std::array<Point<2>, 3>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0,
                                                  ...
                                                  mapping0,
                                                  mapping1);
    return QGaussSimplex<2>(degree).mapped_quadrature<2, 2>(vec_of_simplices);
  }
...
#  if defined(CGAL_GEQ_515)
  ...
  compute_quadrature_on_intersection<3, 1, 3>(
    const typename dealii::Triangulation<3, 3>::cell_iterator &cell0,
    const typename dealii::Triangulation<1, 3>::cell_iterator &cell1,
    const unsigned int                                         degree,
    ...
    const Mapping<1, 3>                                       &mapping1)
  {
    const std::vector<std::array<Point<3>, 2>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0,
                                                  ...
                                                  mapping0,
                                                  mapping1);
    return QGaussSimplex<1>(degree).mapped_quadrature<1, 3>(vec_of_simplices);
  }
  ...
  compute_quadrature_on_intersection<3, 2, 3>(
    const typename dealii::Triangulation<3, 3>::cell_iterator &cell0,
    const typename dealii::Triangulation<2, 3>::cell_iterator &cell1,
    const unsigned int                                         degree,
    const Mapping<3, 3>                                       &mapping0,
    const Mapping<2, 3>                                       &mapping1)
  {
    const std::vector<std::array<Point<3>, 4>> &vec_of_simplices =
      CGALWrappers::compute_intersection_of_cells(cell0,
                                                  ...
                                                  mapping0,
                                                  mapping1);
    return QGaussSimplex<2>(degree).mapped_quadrature<2, 3>(vec_of_simplices);
  }
#  else
  template <>
  dealii::Quadrature<3>
...
#  endif
  ...
  compute_quadrature_on_intersection<3, 3, 3>(
    const typename dealii::Triangulation<3, 3>::cell_iterator &cell0,
    const typename dealii::Triangulation<3, 3>::cell_iterator &cell1,
    const unsigned int                                         degree,
    const Mapping<3, 3>                                       &mapping0,
    const Mapping<3, 3>                                       &mapping1)
  {
    ...
    CGALWrappers::dealii_cell_to_cgal_surface_mesh(cell0, mapping0, surface_1);
    CGALWrappers::dealii_cell_to_cgal_surface_mesh(cell1, mapping1, surface_2);
    ...
    CGALWrappers::compute_boolean_operation(
      surface_1,
      surface_2,
      CGALWrappers::BooleanOperation::compute_intersection,
      out_surface);
    ...
    return QGaussSimplex<3>(degree).mapped_quadrature<3, 3>(vec_of_simplices);
  }
