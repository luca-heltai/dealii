...
// Test basic properties of FEInterfaceValues with hp::FECollection and
// hp::QCollection
...
test(const unsigned int p)
{
  ...
  // Assign different finite elements spaces on different cells
...
{
  initlog();
  deallog << "Testing in 2D" << std::endl;
  for (unsigned int p : {1, 2})
    test<2>(p);
  deallog << "Testing in 3D" << std::endl;
