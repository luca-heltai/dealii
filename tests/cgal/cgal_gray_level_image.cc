...
main()
{
  ...
  const std::vector<std::string> fnames{SOURCE_DIR "/input_grids/skull_2.9.inr",
                                        SOURCE_DIR "/input_grids/liver.inr"};
  int                            counter = 1;
  for (const auto &filename : fnames)
    {
      GridIn<3> grid_in;
      ...
      data.facet_angle            = 30.;
      data.facet_size             = 6.;
      data.facet_distance         = 2.;
      data.cell_radius_edge_ratio = 3.;
      data.cell_size              = 8.;
      grid_in.read_inr(filename, data);
      std::ofstream out_name("gray_level_image" + std::to_string(counter) +
                             ".vtk");
      GridOut().write_vtk(tria, out_name);
      // If we got here, everything went fine.
      deallog << "OK" << std::endl;
      // remove("gray_level_image.vtk");}
      ++counter;
      tria.clear();
    }
}
