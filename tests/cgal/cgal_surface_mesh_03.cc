...
test()
{
  ...
  const std::vector<BooleanOperation> bool_operations{
    ...
    BooleanOperation::difference12,
    BooleanOperation::difference21,
    BooleanOperation::corefine};

  for (const auto &bool_op : bool_operations)
    {
      compute_boolean_operation(sm0, sm1, outsm, bool_op);
      ...
      // (bool_op == BooleanOperation::corefine) ? deallog << sm0 << std::endl :
      //                                           deallog << outsm <<
      //                                           std::endl;
      // Clear surface
      outsm.clear();
