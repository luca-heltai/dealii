...
/**
 * Test prolongation with non-nested levels.
 *
 * Example:
 *
 * +---+---+-----+      +-----+-----+
...
template <int dim, typename Number>
void
other_test(const FiniteElement<dim> &   fe_fine,
           const FiniteElement<dim> &   fe_coarse,
           const Function<dim, Number> &function)
{
  // create coarse grid
...
test(int fe_degree, const Function<dim, Number> &function)
{
  ...
  if (fe_degree > 0)
    {
      ...
      other_test<dim, double>(FE_Q<dim>(fe_degree),
                              FE_Q<dim>(fe_degree),
                              function);
      deallog.pop();
    }
  ...
  //     other_test<dim, double>(FE_DGQ<dim>(fe_degree),
