         ...
         const MGTransferType &                mg_transfer)
{
  ...
  for (unsigned int level = min_level; level <= max_level; ++level)
    {
      ...
      // Compute inverse diagonal of the operator and write it to
      // smoother_data[level].preconditioner->get_vector()
      // mg_matrices[level].compute_inverse_diagonal(
      //   smoother_data[level].preconditioner->get_vector());
      auto &prec_vec = smoother_data[level].preconditioner->get_vector();
      prec_vec.reinit(mg_matrices[level].m());
      for (unsigned int i = 0; i < mg_matrices[level].m(); ++i)
        {
          auto dm     = mg_matrices[level].diag_element(i);
          prec_vec[i] = (std::abs(dm) > 1.0e-10) ? (1.0 / dm) : 1.0;
        }
  ...
  if (mg_data.coarse_solver.type == "cg")
  ...
  else if (mg_data.coarse_solver.type == "cg_with_chebyshev")
    {
      ...
      // Compute inverse diagonal of the operator and write it to
      auto &prec_vec = smoother_data.preconditioner->get_vector();
      prec_vec.reinit(mg_matrices[min_level].m());
      for (unsigned int i = 0; i < mg_matrices[min_level].m(); ++i)
        {
          auto dm     = mg_matrices[min_level].diag_element(i);
          prec_vec[i] = (std::abs(dm) > 1.0e-10) ? (1.0 / dm) : 1.0;
        }
  ...
  else if (mg_data.coarse_solver.type == "cg_with_amg")
    {
      ...
      // CG with AMG as preconditioner
      precondition_amg.initialize(mg_matrices[min_level], amg_data);
