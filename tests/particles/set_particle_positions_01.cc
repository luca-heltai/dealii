...
test()
{
  ...
  const auto set =
    Particles::Utilities::locally_relevant_ids(particle_handler, spacedim);


  TrilinosWrappers::MPI::Vector vector(set, MPI_COMM_WORLD);
  Particles::Utilities::get_particle_positions(particle_handler, vector);
  deallog << "Local set: ";
  set.print(deallog);
  deallog << std::endl
  ...
  vector *= 1e-3;
  Particles::Utilities::set_particle_positions(vector, particle_handler);
  ...
  Particles::Utilities::get_particle_positions(particle_handler, new_vector);
