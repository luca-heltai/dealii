...
template <typename Value,
          ...
          typename Box,
          typename Allocators>
struct TestLevelVisitor : public boost::geometry::index::detail::rtree::visitor<
                            Value,
 ...
 * TestLevelVisitor class. It is used in step-70.
 *
...
template <typename Value,
          ...
          typename Box,
          typename Allocators>
TestLevelVisitor<Value, Options, Translator, Box, Allocators>::TestLevelVisitor(
...
template <typename Value,
          ...
          typename Allocators>
void
TestLevelVisitor<Value, Options, Translator, Box, Allocators>::operator()(
  const TestLevelVisitor::InternalNode &node)
{
  using ElmentsType =
  ...
  if (level == target_level)
    {
      ...
      for (typename ElmentsType::const_iterator it = elements.begin();
           ...
           ++it)
        {
          ...
          // boost::geometry::index::detail::utilities::print_indexable(std::cout,
          //                                                            it->first);
          // IndexableGetterFromIndices<decltype(seconds)> getter(seconds);
          // const auto &p = getter(0); // returns
          // point_temperature[i].first;
...
void
TestLevelVisitor<Value, Options, Translator, Box, Allocators>::operator()(
  const TestLevelVisitor::Leaf &leaf)
{
  using ElmentsType =
    typename boost::geometry::index::detail::rtree::elements_type<Leaf>::type;
  const auto &elements = boost::geometry::index::detail::rtree::elements(leaf);

  std::cout << "LEAF - L:" << level << " V:" << elements.size()
            << " @:" << &leaf << std::endl;
  for (typename ElmentsType::const_iterator it = elements.begin();
       it != elements.end();
       ++it)
    {
      ...
      // unsigned int provafirst = (*it).first; // Errore perche' e' una BBox e
      // non un intero
      unsigned int provasecond = (*it).second;
      // std::cout << (*it).second << std::endl;
    }
}
...
extract_rtree_level(const Rtree &tree, const unsigned int level)
{
  ...
  if (rtv.depth() == 0)
  ...
  else
    {
      ...
      TestLevelVisitor<typename RtreeView::value_type,
