...
namespace NonMatching
{
   ...
   * v_i(x) \delta^{\epsilon}(x-y) w_\alpha(y) dx \ dy,
   ...
   * defaults to StaticMappingQ1 for both.
   *
   * This function will also work in parallel, provided that the second
   * triangulation is of type parallel::shared::Triangulation<dim1,spacedim>.
   * An exception is thrown if you use a slave
   * parallel::distributed::Triangulation<dim1,spacedim>.
   *
   * This method assumes that you make a convolution with a function that has
   * support in `[-epsilon, epsilon]`.
   *
              ...
              for (const auto &entry : intersection)
                {
                  ...
                  constraints.add_entries_local_to_global(dofs0,
                                                          dofs1,
                                                          sparsity);
                }
            }
   ...
   * parallel::distributed::Triangulation<dim1,spacedim>.
   *
   * The one dimensional function @p kernel you pass to this method must have
   * compact support equal to $[-1,1]$. The convolution is made with the
   * function
   *
   * \f[
   * \delta^\epsilon(r) := \frac{1}{\epsilon^d} \prod_{i=1}^{d}
   *  K\left(\frac{r_i}{\epsilon}\right),
   * \f]
   *
   * where $K$ is the function @p kernel, and $\epsilon$ is the parametr
   * @p epsilon, representing the size of the support used to compute the
   * convolution.
   *
   * @author Luca Heltai, 2019.
   */
