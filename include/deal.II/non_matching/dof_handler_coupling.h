...
namespace NonMatching
{
  ...
  public:
...

    {
      ...
      n_comps = std::min(comps1.n_selected_components(),
                         comps2.n_selected_components());

      if (auto tr = dynamic_cast<
            const parallel::distributed::Triangulation<spacedim> *>(&(*tria1)))
        {
                      ...
                      for (unsigned int j = 0; j < fe1->dofs_per_cell; ++j)
                        {
                          ...
                          if (comp_i == comp_j)
                            local_matrix(i, j) += fe1->shape_value(i, ref_q) *
          ...
          if (reuse_internal_data_structures == false || !particle_handler)
            {
              ...
              unsigned int j = 0;
              for (unsigned int c = first_comp;
                   c < comps2.size() && j < n_comps;
                   ++c)
                if (comps2[c])
