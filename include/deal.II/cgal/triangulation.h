...
#  ifndef DOXYGEN
                                  ...
                                  const CGALWrappers::AdditionalData<3> &data)
  {
    ...
    dealii_tria_to_cgal_surface_mesh(surface_tria, mesh);
    ...
    cgal_triangulation_to_dealii_triangulation(cgal_triangulation, vol_tria);
  }
} // namespace CGALWrappers
#  endif // doxygen
...
namespace CGALWrappers
{
  ...
  implicit_function(Triangulation<dim, 3> &    tria,
                    const Function<3> &        implicit_function,
                    const AdditionalData<dim> &data = AdditionalData<dim>{},
                    const Point<3> &           interior_point    = Point<3>(),
                    const double &             outer_ball_radius = 1.0)
  {
    (void)tria;
  ...
  surface_mesh_to_volumetric_mesh(
    const Triangulation<2, 3> &surface_tria,
    Triangulation<3> &         vol_tria,
